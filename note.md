...
## 563二叉树的坡度
1. 遍历。定义递归函数计算子树和，在后序位置计算坡度。

## 572另一棵树的子树
1. 遍历。遍历每一个节点，将每个节点都与subTree对比，使用辅助函数isSameTree(TreeNode* p, TreeNode* q)判断是否相等

## 606根据二叉树创建字符串
1. 遍历。在后序位置组装左右字符串，同时需要看懂题目要求，转化为编程语言。在节点只有一边子树的情况下，要求一：左子树存在，省略右子树；要求二：右子树存在，不能省略左子树。

## 617合并二叉树
1. 遍历。遍历的过程中，对每个节点做什么。由于是修改二叉树，需要用一个变量去接收递归的返回值。

## 623在二叉树中增加一行
1. 层次遍历。在for循环中能够拿到每一层的节点，设计辅助函数进行新节点的插入。
2. 深度遍历。借助外部变量记录深度，当遍历到目标层数时，进行节点插入。

## 637二叉树的层平均值
1. 层次遍历。记录每一层的总和，层次遍历在for循环中能知道当前层有多少个节点。

## 653两数之和 IV - 输入 BST
1. 深度遍历。利用hash表加快差的查找，遍历每一个节点。
2. 中序遍历。利用中序遍历得到BST的有序数组，利用双指针找到两数之和。

## 655输出二叉树
1. 深度遍历。找到当前节点行列号和其父节点行列号的关系。

## 662二叉树最大宽度
1. 层次遍历。
2. 深度遍历。关键在于给每个节点编号，假设父节点的编号是 x，左子节点就是 2 * x，右子节点就是 2 * x + 1。
